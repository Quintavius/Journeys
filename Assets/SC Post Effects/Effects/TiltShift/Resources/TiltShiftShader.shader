Shader "Hidden/SC Post Effects/Tilt Shift"
{
	...
	float4 FragHorizontal(VaryingsDefault i) : SV_Target
	{
		...
		float mask = HorizontalMask(i.texcoord.xy) * 0.01;

		float4 poissonScale = mask * _Params.z;

		for (int l = 0; l < SmallDiscKernelSamples; l++)
		{
			float2 sampleUV = i.texcoordStereo.xy + SmallDiscKernel[l].xy * poissonScale.xy;
	...
	float4 FragHorizontalHQ(VaryingsDefault i) : SV_Target
	{
		float3 sum = 0;
	...
	float4 FragRadial(VaryingsDefault i) : SV_Target
	{
		...
		float mask = RadialMask(i.texcoord.xy) * 0.01;
	...
	float4 FragRadialHQ(VaryingsDefault i) : SV_Target
	{
		float3 sum = 0;
	...
	{
		float mask = _Params.w == 0 ? HorizontalMask(i.texcoord.xy) : RadialMask(i.texcoord.xy);
