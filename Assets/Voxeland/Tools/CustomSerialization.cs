		...
		public static int WriteClass (object obj, List<string> classes, List<UnityEngine.Object> objects, List<float> floats, List<object> references)
		{
			...
			foreach (Value val in Values(obj))
			{
				...
				else if (val.type == typeof(Keyframe))
				{
					...
					writer.WriteLine("\t<" + val.name + " type=" + val.type + " time=" + k.time + " value=" + k.value + 
						" in=" + k.inTangent + " out=" + k.outTangent + 
			...
			while (true)
			{
				...
				else
				{
					...
					else if (val.type == typeof(Keyframe))
					{
						...
						#if UNITY_2018_2_OR_NEWER
						k.inWeight = (int)lineMembers[7].Parse(typeof(float));
						k.outWeight = (int)lineMembers[8].Parse(typeof(float));
						#else
